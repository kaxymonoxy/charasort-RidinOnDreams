...
The version in this repo is built for Ridin on Dreams (Hololive 7th fes) performances, but the sorter can be easily edited to create any custom sorter desired. I have a sorter for the previous fes (6th, Color Rise Harmony) in my repos, which i no longer host on a site, but you can download and run it for yourself since it works entirely client side. This is the only line in the README written by me, everything else is from execfera.
